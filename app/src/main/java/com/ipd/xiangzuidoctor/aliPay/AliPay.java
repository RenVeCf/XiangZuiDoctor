package com.ipd.xiangzuidoctor.aliPay;
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        ApplicationUtil.getContext().startActivity(new Intent(ApplicationUtil.getContext(), AuthenticationResultActivity.class).putExtra("result_type", 5).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
                    } else {
                        // 该笔订单真实的支付结果，需要依赖服务端的异步通知。
                        showAlert(activity, activity.getString(R.string.pay_failed) + payResult);
                        ApplicationUtil.getContext().startActivity(new Intent(ApplicationUtil.getContext(), AuthenticationResultActivity.class).putExtra("result_type", 6).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
                    }
                    break;
                }
                case SDK_AUTH_FLAG: {
